...
func (v versionCommand) Help() string {
	return `Display the current version of tanglesctl, and check for updates.`
}

func (v versionCommand) Run(args []string) int {
	fmt.Printf("tanglesctl version %s\n", v.curVersion)
	...
	if curVersion.ImpracticalUpdates.Deprecated {
		urgent = fmt.Sprintf("[!] Your current version of tanglesctl, %s, is deprecated. ", curVersion.ImpracticalUpdates.Version)
		recommended = " It is *highly* recommended that you update as soon as possible."
	}
	fmt.Printf("%sA newer version of tanglesctl, %s, is available for download now.%s To get it, head to %s.\n", urgent, newVersion.ImpracticalUpdates.Version, recommended, newVersion.URL)
	return 0
}

func (v versionCommand) Synopsis() string {
	return "Display the current version of tanglesctl and check if a newer version is available."
}
